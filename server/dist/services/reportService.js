...
export const getWorkerPeriodReportsService = async ({ workerName, start, end }) => {
...
export const getObjectPeriodReportsService = async ({ objectName, start, end }) => {
    const startDate = new Date(start);
    const endDate = new Date(end);
    validateDates(startDate, endDate);
    const reports = await Report.find({
    ...
    for (const report of reports) {
        if (!report.analysis?.workers?.length)
            continue;
